...
inputEl.addEventListener('focus', () => setOutput('')); // optional nicety
...
function onConvert() {
    ...
    // reject non-numeric and decimal input (allow leading zeros if you prefer: /^-?0*\d+$/)
    if (!/^-?\d+$/.test(raw)) {
        setOutput('Please enter a valid number');
        return;
    }
